...
export function InboxView({ onNavigate }: { onNavigate: (view: ViewId) => void }) {
  ...
  useEffect(() => {
    if (focusNotificationId && items.some((item) => item.id === focusNotificationId)) {
      setSelectedId(focusNotificationId)
      markNotificationRead(focusNotificationId)
    ...
    return items
      .filter(
        (item) =>
          ...
          [item.title, item.preview, item.club, item.senderName, ...item.body]
            .join(" ")
            .toLowerCase()
            .includes(q),
      )
      .sort((a, b) => {
        if (sort === "priority")
          return notificationPriority(a) - notificationPriority(b) || date(b) - date(a)
  ...
  const groups = notificationGroups(filtered, sort === "club", sort === "priority")
    ...
    <div className="mx-auto max-w-6xl space-y-6 text-foreground">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-neutral-500" role="status">
          {unread} unread · {items.length} notifications
        </p>
        <Button
          variant="outline"
          size="sm"
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative min-w-52 flex-1">
            ...
            <Input
              ...
              placeholder="Search notifications, clubs, or keywords"
              value={query}
          ...
          <select
            ...
            className={cn(selectStyle, "max-w-full sm:max-w-60")}
            value={club}
          ...
          >
            <option value="priority">Recruiting priority</option>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {filters.map((value) => (
            <button
              ...
              className={cn(
                "rounded-md px-3 py-2 text-xs font-medium transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring",
                filter === value
          ...
          {(query || filter !== "All" || club !== "all" || readFilter !== "all") && (
      ...
      <div
        className={cn(
          ...
          selected && "lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]",
        )}
      >
        <section
          aria-label="Notification list"
          className={cn("min-w-0 overflow-hidden", selected && "hidden lg:block")}
        >
          <div className="flex min-h-14 flex-wrap items-center gap-3 border-b border-neutral-200 px-4 py-3">
            <Checkbox
              ...
              onCheckedChange={() =>
                setChecked(allChecked ? new Set() : new Set(filtered.map((item) => item.id)))
              }
            />
            <span className="text-xs text-neutral-500" role="status">
              {checkedIds.length ? `${checkedIds.length} selected` : `${filtered.length} results`}
            </span>
            {checkedIds.length > 0 && (
              <div className="ml-auto flex flex-wrap gap-1">
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    setNotificationsRead(checkedIds, true)
                    setChecked(new Set())
                  }}
                >
                  Mark read
                ...
                <Button
                  ...
                  variant="ghost"
                  onClick={() => {
                    setNotificationsRead(checkedIds, false)
                    setChecked(new Set())
                  }}
                >
                  Mark unread
                ...
                <div className="divide-y divide-border">
                  {group.items.map((item) => (
                    <div
                      key={item.id}
                      className={cn(
                        "flex items-start gap-3 border-l-2 border-transparent px-3 py-5 motion-safe:animate-in motion-safe:fade-in motion-safe:transition-colors motion-safe:duration-200 hover:bg-secondary/40",
                        selectedId === item.id && "bg-secondary/50",
                        !item.read && "border-l-primary bg-secondary/20",
                      )}
                    >
                      <Checkbox
                        className="mt-1"
                      ...
                      <button
                        ...
                        className="min-w-0 flex-1 space-y-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-neutral-400"
                      >
                        ...
                        <div className="flex flex-wrap items-center gap-2 text-[11px] text-neutral-500">
                          ...
                          <time dateTime={item.createdAt}>
                            {item.fullDate === "Just now" && item.createdAt
        ...
        <section
          ...
          className={cn("min-w-0 lg:border-l lg:border-border lg:pl-6", !selected && "hidden")}
        >
          {!selected ? (
            <div className="flex min-h-96 flex-col items-center justify-center gap-3 p-8 text-center text-neutral-500">
              <Bell className="size-8 text-neutral-300" />
              <p className="text-sm">Choose a notification to read the full update.</p>
            </div>
          ) : (
            <>
              <div className="space-y-5 border-b border-neutral-200 p-5 sm:p-6">
                      ...
                      if (previous)
                        document
                          .querySelector<HTMLButtonElement>(
                            `[data-notification-id="${CSS.escape(previous)}"]`,
                          )
                          ?.focus()
                    })
                  }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <p className="text-sm font-semibold">{selected.club}</p>
                    <p className="text-xs text-neutral-500">{selected.senderTitle}</p>
                  </div>
                </div>
                <div>
                  <p className="mb-2 text-xs text-neutral-500">
                    {selected.type} · {selected.fullDate}
                  </p>
                  <h2
                    ref={detailHeading}
                    tabIndex={-1}
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setNotificationsRead([selected.id], !selected.read)}
                  >
                    {selected.read ? <Mail className="size-4" /> : <MailOpen className="size-4" />}
                ...
                {selected.locationChange && (
                  <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-4">
                    <p className="flex items-center gap-2 text-sm font-medium">
                      ...
                      {selected.locationChange.newLocation}
                    </p>
                    <p className="mt-1 text-xs text-neutral-500">
                      Previously: {selected.locationChange.oldLocation}
                    </p>
                  </div>
                )}
                {(selected.eventId ||
                  ...
                  <Button
                    variant="outline"
                    onClick={() => {
                      if (selected.eventId) focusEvent(selected.eventId)
                      onNavigate("calendar")
                    }}
                  >
