...
function load(file, prisma, roles) {
 ...
 const mocks={'@/utils/prisma':{prisma},'@/utils/auth':{requireClubRole:async(id,allowed)=>{roles.push([id,allowed]);return {membership:{id:'reviewer'}}}},'next/cache':{revalidatePath:()=>{}}}
...
test('round moves reject foreign rounds before any mutation',async()=>{
 ...
 assert.deepEqual(roles[0],[clubId,['PRESIDENT','RECRUITMENT_LEAD']])
})
test('status updates are president-only and scope mutations to non-draft applications in the club',async()=>{
...
test('evaluation reads enforce club isolation within the database query',async()=>{
 const action=load('actions/evaluations.ts',{evaluation:{findMany:async args=>{assert.deepEqual(args.where,{applicationId,application:{clubId,status:{not:'DRAFTING'}}});return []}}},[])
...
test('valid evaluations preserve server scoring and reviewer identity',async()=>{
 const roles=[];let payload;const action=load('actions/evaluations.ts',{application:{findFirst:async()=>({id:applicationId})},pipelineRound:{findFirst:async()=>({id:newRoundId})},evaluation:{upsert:async args=>{payload=args;return {id:'eval'}}}},roles)
